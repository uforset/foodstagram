			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<hr />
						<h5>신고 내용</h5>
						<div id="reportContents" class="text-center p-5"></div>
					</div>
			...
			<div id="list-guestbook" class="">
				<c:forEach items="${firstList}" var="chat">
					...
					<c:if test="${loginMember.memberId eq chat.memberId}">
						...
						<li data-no="${chat.no}" class="me pr-2"><strong class="">${chat.memberId}</strong>
							<div class="me ">

								<strong style="display: inline;" class="align-self-end"><fmt:formatDate
										value="${chat.sendDate }" pattern="yy/MM/dd HH:mm" /></strong>
								<c:if test="${chat.vaildYN eq 'Y'}">

									<p class=" myChat text-muted p-2">
										<b>신고된 채팅입니다.</b>
									</p>
					...
					<c:if test="${loginMember.memberId ne chat.memberId}">
						...
						<li data-no="${chat.no}" class="pl-2"><strong>${chat.memberId}</strong>
							<div class="row ml-0">
								<c:if test="${chat.vaildYN eq 'Y'}">
									<p class="otherChat bg-light text-muted p-2">
										<b>신고된 채팅입니다.</b>
									</p>
								</c:if>
								<c:if test="${chat.vaildYN ne 'Y'}">
									...
									<strong class="align-self-center"><fmt:formatDate
											value="${chat.sendDate }" pattern="yy/MM/dd HH:mm" /> <a
										href='#' class='reportModalK'>신고</a></strong>
								</c:if>
							</div></li>
					</c:if>
				</c:forEach>
			...
			.ready(
					function() {
						...
						var fetchList = function() {
							...
							var endNo = $("#list-guestbook li").first().data(
									"no") || 0;
						...
						var renderList = function(vo, endNo) {
							//alert("아뭐냐구");
							// 리스트 html을 정의
							var date = moment(vo.sendDate).format(
									'YY/MM/DD HH:mm');
							var html = "";
							if (endNo == 0)
								endNo = vo.no;
							...
							if (vo.memberId == "${loginMember.memberId}") {
								...
								if (vo.vaildYN != 'Y') {
									content = "<p class='myChat text-left p-2'>"
											+ vo.chatContent + "</p>";
								}
							...
							//남이 보낸 채팅일 경우
								...
								if (vo.vaildYN != 'Y') {
									content = "<p class='otherChat bg-light p-2'>"
											+ vo.chatContent + "</p>";
									report = "신고";
								}
						...
						$(function() {
							...
							client.connect({}, function() {
								...
								//	           일반메세지 들어오는곳         
								client
										.subscribe('/subscribe/chat/' + roomNo,
												function(chat) {
													//받은 데이터
													var content = JSON
															.parse(chat.body);
								...
								//신고내용 들어오는곳
								client.subscribe('/subscribe/report/' + roomNo,
										function(report) {
											//받은 데이터
											var content = JSON
													.parse(report.body);
