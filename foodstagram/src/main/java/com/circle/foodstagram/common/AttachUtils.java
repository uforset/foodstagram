...
public class AttachUtils {
	...
	// 보드어태치 전용 메소드
	/** 다중 MultipartFile에서 VO 설정 및 업로드 파일 처리 후 List 리턴 */
	public List<BoardAttach> getBoardAttachListByMultiparts(MultipartFile[] boFiles, String path, HttpServletRequest request)
			throws IOException {
		List<BoardAttach> atchList = new ArrayList<BoardAttach>();
		//String savePath = request.getSession().getServletContext()
		uploadPath = request.getSession().getServletContext().getRealPath("resources");
		for (int i = 0; i < boFiles.length; i++) {
			MultipartFile multipart = boFiles[i];
			BoardAttach vo = this.getBoardAttachByMultipart(multipart, path);
			vo.setAtch_seq(i + 1);
			if (vo != null) {
				atchList.add(vo);
			}
	...
	/** MultipartFile에서 VO 설정 및 업로드 파일 처리 후 리턴, 없는 경우 null */
	public BoardAttach getBoardAttachByMultipart(MultipartFile multipart, String path) throws IOException {
		if (!multipart.isEmpty()) {
			int pos = multipart.getOriginalFilename().lastIndexOf(".");
	        String ext = multipart.getOriginalFilename().substring(pos + 1);
			String fileName = UUID.randomUUID().toString() + "." + ext; // 랜덤uid + 확장자 붙여줌
			BoardAttach vo = new BoardAttach();
			vo.setAtch_original_name(multipart.getOriginalFilename());
			vo.setAtch_file_size((int)multipart.getSize());
			vo.setAtch_content_type(multipart.getContentType());
			vo.setAtch_file_name(fileName);
			vo.setAtch_fancy_size(fancySize(multipart.getSize()));
	...
	/** 다중 Attach 에서 각각 업로드한 파일 삭제 처리후 List<Integer>리턴(atch_no) */
		public List<Integer> getNoticedeletAtchNoList(List<NoticeAttach> attaches, String path, HttpServletRequest request) {
			List<Integer> atchNoList = new ArrayList<Integer>();
			uploadPath = request.getSession().getServletContext().getRealPath("resources");
			for(NoticeAttach a : attaches) {
				atchNoList.add(noticeDeleteFile(a, path)); // 파일 삭제후 리시트에 넣어줌
			}
			return atchNoList; //삭제목록 (atch_no) 리턴
		}
		
		/** 업로드한 파일 삭제처리후 atch_no 리턴 */
		public int noticeDeleteFile(NoticeAttach noticeattach, String path) {
			File file = new File(uploadPath + File.separatorChar + path + File.separatorChar + noticeattach.getAtch_file_name());
			if(file.exists()) {// 파일이존재하면
				log.info(noticeattach.getAtch_file_name() + "파일 삭제.");
				log.info(noticeattach.getAtch_original_name() + "파일 삭제.");
				file.delete(); // 파일 삭제	
			}
			return noticeattach.getAtch_no();
		}
		
		// notice 어태치 전용 메소드
		/** 다중 MultipartFile에서 VO 설정 및 업로드 파일 처리 후 List 리턴 */
		public List<NoticeAttach> getNoticeAttachListByMultiparts(MultipartFile[] boFiles, String path, HttpServletRequest request)
				throws IOException {
			List<NoticeAttach> atchList = new ArrayList<NoticeAttach>();
			//String savePath = request.getSession().getServletContext()
			uploadPath = request.getSession().getServletContext().getRealPath("resources");
			for (int i = 0; i < boFiles.length; i++) {
				MultipartFile multipart = boFiles[i];
				NoticeAttach vo = this.getNoticeAttachByMultipart(multipart, path);
				if (vo != null) {
					atchList.add(vo);
				}
			}
		...
		/** MultipartFile에서 VO 설정 및 업로드 파일 처리 후 리턴, 없는 경우 null */
		public NoticeAttach getNoticeAttachByMultipart(MultipartFile multipart, String path) throws IOException {
			if (!multipart.isEmpty()) {
				int pos = multipart.getOriginalFilename().lastIndexOf(".");
		        String ext = multipart.getOriginalFilename().substring(pos + 1);
				String fileName = UUID.randomUUID().toString() + "." + ext; // 랜덤uid + 확장자 붙여줌
				NoticeAttach vo = new NoticeAttach();
				//vo.orsetAtchOriginalName(multipart.getOriginalFilename());
				vo.setAtch_original_name(multipart.getOriginalFilename());
				//vo.setAtchFileSize(multipart.getSize());
				vo.setAtch_file_size(multipart.getSize());
